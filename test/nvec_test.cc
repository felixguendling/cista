...
TEST_CASE("nvec ranges test") {
  ...
  auto const expected_lhs = std::vector<std::vector<std::vector<unsigned>>>{
      {{1U, 2U}, {3U}},
      {{4U}},
  };
  ...
  auto const rows_match = std::ranges::equal(
      zipped, expected_zip,
      [](auto const& actual_pair, auto const& expected_pair) {
        auto const& [lhs_rows, rhs_bucket] = actual_pair;
        auto const& [lhs_expected_rows, rhs_expected_bucket] = expected_pair;
        auto const lhs_match = std::ranges::equal(
            lhs_rows, lhs_expected_rows,
            [](auto const& lhs_row_bucket, auto const& rhs_row_bucket) {
              return std::ranges::equal(lhs_row_bucket, rhs_row_bucket);
            });
        auto const rhs_match =
            std::ranges::equal(rhs_bucket, rhs_expected_bucket);
