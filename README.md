...
The following functions exist in `cista::offset` and `cista::raw`:
  ...
  - **`void deserialize<Container>(Container&)`** deserializes an object from a `std::vector<uint8_t>` or similar data structure. This function throws a `std::runtimer_error` if the data is not well-formed. functions as described [here](#serialization).
  - **`void deserialize(uint8_t* from, uint8_t* to)`** deserializes an object from a pointer range. This function throws a `std::runtimer_error` if the data is not well-formed.
  - **`void unchecked_deserialize<Container>(Container&)`** deserializes an object from a `std::vector<uint8_t>` or similar data structure. No checking is performed!
  - **`void unchecked_deserialize(uint8_t* from, uint8_t* to)`** deserializes an object from a pointer range. No checking is performed!
