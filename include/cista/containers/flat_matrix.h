...
struct base_flat_matrix {
  ...
  struct row {
    row(base_flat_matrix& matrix, int const row_index) noexcept
  ...
  struct const_row {
    const_row(base_flat_matrix const& matrix, int const row_index) noexcept
        : matrix_(matrix), row_index_(row_index) {}
...
template <typename V>
inline auto make_flat_matrix(std::uint32_t const column_count,
