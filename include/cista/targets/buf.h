...
struct buf {
  ...
  uint8_t* addr(offset_t const offset) noexcept { return (&buf_[0U]) + offset; }
  uint8_t* base() noexcept { return &buf_[0U]; }

  uint64_t checksum(offset_t const start = 0U) const noexcept {
  ...
  offset_t write(void const* ptr, std::size_t const num_bytes,
    ...
    if (alignment > 1U && buf_.size() != 0U) {
      ...
      auto const new_offset = static_cast<offset_t>(
          aligned_ptr ? static_cast<uint8_t*>(aligned_ptr) - base() : 0U);
